...
export class RestService {
  ...
  public loginUser(user: Customer): Observable<Customer> {
    return this.http.post<Customer>("http://localhost:8181/login", user, { headers: this.requestHeaders });
  }
  ...
  updateUser(id: number, phn: string) {
    return this.http.put(`http://localhost:8181/updateUser/${id}/${phn}`, { id, phn })
  }
...
http://localhost:8383/email?tomail=${tomail}&subject=${sub}&body=${body}`, { tomail, sub, body });
  }
